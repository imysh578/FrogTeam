...
| 페이지 | 화면 | 설명 |
| --- | --- | --- |
| **메인페이지** |  이미지 | NavBar에는 각 섹션마다 이동을 가능하게 하고 HeroSection 에는 스크롤 형식으로 각 섹션 이동후 각페이지로 이동하는 버튼을 만들었다. 스크롤을 내리면 맨위로 올리는 버튼도 구현하였다. |
| **로그인/회원가입** | 이미지 | 이메일과 비밀번호를 DB에 저장하며 간단한 회원가입과 로그인을 가능케 하였다.|
| **마이페이지** | 이미지 | 회원가입후 개인 회원이 자신이 거래한 거래소의 API 를 가져와 자신의 자산을 확인하고 자산관리를 할수있게끔 하였다.|
| **뉴스** | 이미지 | 뉴스 API로 트렌딩 되는 가상화폐관련된 뉴스만 모아 뉴스 섹션에 정보를 제공하고있다.|
| **차트** | 이미지 | 업비트와 바이낸스 API를 활용하여 가상화폐의 가격을 실시간으로 확인할수있게 하였다. |
| **ETF** | 이미지 | 안정적인 투자를 가능하게 해주는 ETF의 정보를 제공해주는 API를 활용하여 각 ETF 상품의 세부설명을 보여주는 ETF 검색사이트에 링크를 걸어 놓았다.|
| **게시판** | 이미지 | 회원들이 게시판에 정보를 올리는 게시판을 구현하였다.|
...
- [상황]  
리덕스를 사용하자는 늦은 결정으로 인해 
Nav, Header, HeroSection, InfoSection 등의 Components들을 리덕스를 활용 못하고 구현하게 되었다.
- [문제]  
Redux Store 를 사용하기엔 이미 늦어버렸다.
처음부터 전부 다시 해야하는 상황이었다.
